    ...
    try {
        ...
        if (!fs.existsSync(credsPath) || conf.session !== "zokk") {
            ...
            if (/^[a-zA-Z0-9]+_[a-z0-9]{5}$/i.test(conf.session)) {
                const sessionUrl = `https://bcuieirreeidjtvhzvlg.supabase.co/storage/v1/object/public/session-data/${conf.session}.json`;
        ...
        adams.ev.on("messages.upsert", async ({ messages }) => {
            ...
            if (verifGroupe && infosGroupe) {
                ...
                isBotOwner = (auteurMessage === ownerJid) || (auteurRegularJid === ownerJid);
                        ...
                        // Let texte be modifiable so we can extract button IDs
            let texte = ms.message?.conversation || 
                         ms.message?.extendedTextMessage?.text || 
                         ms.message?.imageMessage?.caption || 
                         '';

            // CRITICAL STEP 4: Parse interactive button responses
            if (ms.message?.interactiveResponseMessage?.nativeFlowResponseMessage) {
                try {
                    const params = JSON.parse(ms.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson);
                    if (params && params.id) {
                        texte = params.id; // This grabs the hidden command (e.g., ".uptime")
                    }
                } catch (e) {
                    console.error("Error parsing interactive response:", e);
                }
            } else if (ms.message?.templateButtonReplyMessage) {
                texte = ms.message.templateButtonReplyMessage.selectedId;
            } else if (ms.message?.buttonsResponseMessage) {
                texte = ms.message.buttonsResponseMessage.selectedButtonId;
            }
